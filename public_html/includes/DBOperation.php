...
class DBOperation
{
	...
	public function addCustomer($company,$gst,$mobile,$address)
	{
		$pre_stmt=$this->conn->prepare("INSERT INTO `customers`(`company_name`, `gst`, `mobile`, `address`) VALUES (?,?,?,?)");
		$pre_stmt->bind_param("ssss",$company,$gst,$mobile,$address);
		$result=$pre_stmt->execute() or die($this->conn->error);
		if ($result) {
	...
	public function addStock($product_id,$stock_added,$current_stock,$purchase_date)
	{
		$pre_stmt=$this->conn->prepare("INSERT INTO `purchase`(`product_id`, `stock_added`, `purchase_date`) VALUES (?,?,?)");
		$purchase_date = date("Y-m-d", strtotime($purchase_date));
		$pre_stmt->bind_param("iss",$product_id,$stock_added,$purchase_date);
		$result=$pre_stmt->execute() or die($this->conn->error);
		if ($result) 
		{
	...
	public function editCustomer($id,$company,$gst,$mobile,$address)
	{
		$pre_stmt=$this->conn->prepare("UPDATE `customers` SET `company_name`= ?,`gst`= ?,`mobile`=?,`address`= ? WHERE id = ?");
		$pre_stmt->bind_param("ssssi",$company,$gst,$mobile,$address,$id);
		$result=$pre_stmt->execute() or die($this->conn->error);
		if ($result) {
